...
- `projects-load` shows all nine project articles and `Showing 9 of 9 projects`.
...
- `projects-filter-product` presses Product and shows five articles.
- `projects-filter-all` returns to `/projects` and nine articles.
...
- From home, choose `Studio`, `Client`, or `Product` in the nav named `Browse projects by category`. That lands on the filtered URL with the matching chip already pressed.
...
- Viewport `1440×1000`.
- Catalog still has the nine titles listed in `features/README.md`. If `src/data/projects.ts` changed, update the counts in this file before asserting them.
...
- **All state.** Text `Showing 9 of 9 projects` is visible. Role `article` count is `9`. Button `All` has `aria-pressed=true`. The other three filter buttons are `false`. URL has no `category` query.
...
- **Product filter.** Choose `Product`. Run `page.getByRole("button", { name: "Product", exact: true }).click()`. Text `Showing 5 Product projects` is visible. Titles in order are FreeVoiceTranscribe, Apple App Intents Skill, Traces, WealthWise, and Command.AI. URL is `/projects?category=Product`. The Apple card link is named `Get the skill for Apple App Intents Skill (opens in new tab)` and its `project-status` is `Live`.
...
- **Scripted path.** Run `npx playwright test e2e/projects.spec.ts --project=chromium`. That spec asserts `Showing 9 of 9 projects`, clicks Client, asserts the three-article count, and opens the first case study. It does not assert the `?category=` URL. If you use only this spec, also read `page.url()` after the click and record `/projects?category=Client`.
...
- `/projects` renders per request, so curl HTML already contains `Showing 9 of 9 projects` and nine `<article>` tags. That proves the catalog, not the chips. Filter changes and `aria-pressed` still need a hydrated browser.
