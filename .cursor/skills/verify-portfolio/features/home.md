...
Home is the static intro at `/`. A visitor reads the offer, sees four selected project cards, and can go to contact, a filtered category, or the full project list.

## Sub-features

- `home-load` serves the static homepage with the H1, the catalog telemetry, and the availability chip from `lg` up.
...
- `home-category-nav` sends Studio, Client, or Product from the selected-work header to the matching `/projects?category=` view.
...
- **Telemetry.** The hero stats read Studio / `Defendre Solutions`, Showcase / `9 public builds`, Live now / `7 active links`. The two numbers are the catalog row count and the `Live` row count from `features/README.md`. Update them together with the catalog.
...
- **Selected work.** On `/`, the heading `A few I shipped.` is visible, followed by a paragraph that starts `A current sample of studio, client, and product work.` The region `page.getByRole("region", { name: "A few I shipped." })` contains four card links whose accessible names include `Defendre Solutions`, `FreeVoiceTranscribe`, `Apple App Intents Skill`, and `BraidsbyRose`. Those are the first four rows in `src/data/projects.ts`. Traces is not on home. There is no fifth selected card.
- **Category shortcuts.** In the selected-work header, the nav named `Browse projects by category` holds exactly three links: `Studio`, `Client`, `Product`. Run `page.getByRole("navigation", { name: "Browse projects by category" }).getByRole("link", { name: "Client", exact: true }).click()`. The URL is `/projects?category=Client`, the text `Showing 3 Client projects` is visible, and the `Client` filter button has `aria-pressed=true`. Studio and Product follow the same shape and land on `Showing 1 Studio project` and `Showing 5 Product projects`. This is a full document load, not a client transition.
