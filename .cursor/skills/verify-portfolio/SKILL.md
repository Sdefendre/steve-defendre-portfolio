...
Missing, malformed, legacy shell state, unavailable discovery, an orphan/foreign listener, or a changed identity fails closed. No unverified process is signaled, and all state/logs/evidence remain for inspection. State updates write and sync a private temporary file before atomically replacing validated `instance.json`, so a failed update preserves the last complete state. Handled failures remove only their own temporary file; abrupt process termination may leave a partial temporary file for inspection, which future writes do not reuse. Successful cleanup marks JSON state `stopped` and keeps it too. Use a new run ID for another launch. A failed startup may require manual inspection if it never establishes identity/listener ownership; cleanup intentionally refuses to guess. Legacy `instance.env` files are never sourced or migrated automatically.
